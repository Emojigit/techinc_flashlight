...
technic.register_power_tool("technic_flashlight:flashlight", flashlight_max_charge)

minetest.register_alias("technic_flashlight:light_off", "air")
      
minetest.register_tool("technic_flashlight:flashlight", {
...
minetest.register_craft({
	output = "ttechnic_flashlight:flashlight",
	recipe = {
...
local function check_for_flashlight(player)
	...
	for i = 1, 8 do
		if hotbar[i]:get_name() == "technic_flashlight:flashlight" then
...
minetest.register_on_leaveplayer(function(player)
	...
	if nodename == "technic_flashlight:light" then
...
minetest.register_globalstep(function(dtime)
	for i, player in pairs(minetest.get_connected_players()) do
		...
		if was_wielding[player_name] and not flashlight_weared then
			...
			if node and node.name == "technic_flashlight:light" then
		...
		elseif (player_moved or not was_wielding[player_name]) and flashlight_weared then
			...
			if node and node.name == "air" then
				minetest.set_node(rounded_pos, {name="technic_flashlight:light"})
			end
			local node = minetest.get_node_or_nil(old_pos)
			if node and node.name == "technic_flashlight:light" then
				minetest.remove_node(old_pos)
...
minetest.register_node("technic_flashlight:light", {
	drawtype = "glasslike",
